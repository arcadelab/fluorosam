        ...
        for fid, in_channel in zip(fid_list, in_channel_list):
            inputs[fid] = OpSequential(
                [
                    ...
                    UpSampleLayer(size=(out_size, out_size)),
                ]
            )
...
class EfficientViTSamImageEncoder(nn.Module):
    def __init__(
        ...
        self.norm = build_norm("ln2d", embed_dim)
        self.img_size = img_size

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x=F.interpolate(x,size=(self.img_size // 2,self.img_size // 2),mode='bilinear')#mobilesamv2
