...
from ..efficientvit.models.efficientvit.seg import SegHead, EfficientViTSeg
...
def build_prephix_vit_l(checkpoint=None):
    return _build_prephix(
        encoder_embed_dim=1024,
        encoder_depth=24,
        encoder_num_heads=16,
        encoder_global_attn_indexes=[5, 11, 17, 23],
...
def build_prephix_vit_b(checkpoint=None):
    return _build_prephix(
        encoder_embed_dim=768,
        encoder_depth=12,
        encoder_num_heads=12,
...
def build_efficientvit_prephix(checkpoint=None):
    prompt_embed_dim = 512
    image_size = 448
    vit_patch_size = 16
    image_embedding_size = image_size // vit_patch_size
    sam = Sam(
        image_encoder=_build_prephix_encoder(image_size, prompt_embed_dim, image_embedding_size),
        prompt_encoder=PromptEncoder(
            embed_dim=prompt_embed_dim,
            image_embedding_size=(image_embedding_size, image_embedding_size),
            input_image_size=(image_size, image_size),
...
def _build_prephix_encoder(image_size: int, prompt_embed_dim: int, image_embedding_size: int):
    ...
    image_encoder = EfficientViTSamImageEncoder(
        backbone, neck, img_size=image_size, embed_dim=prompt_embed_dim
    )
...
def _build_backbone():
    return EfficientViTLargeBackbone(
        width_list=[32, 64, 128, 256, 512],  # l2 model
...
def build_efficientvit_seg(checkpoint=None):
    image_size = 448
    vit_patch_size = 16
...
def build_efficientvit_l2_encoder(checkpoint=None, img_size=1024):
